...
export const actualizarFacturaHandler = async (event) => {
  ...
  // Si la factura se marca como pagada, actualizar estatus de procedimientos
  if (body.estado_pago === "pagada") {
    let consultas = [];
    try {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({
          error: "Error leyendo consultas_procedimientos",
        }),
      };
    }
    let actualizados = 0;
    for (const consulta of consultas) {
      if (consulta.factura_id === id) {
        consulta.estatus = "pagado";
        actualizados++;
      }
    }
